...
if target == "all":
    for k in FOLDER_PAIRS_LIST.keys():
        playback = PlayBack(fp=FOLDER_PAIRS_LIST[k], cns=None, grid=[3, 1], plot_data_range="all", scaling=[1])
        playback.fig.suptitle(k)

else:
    playback = PlayBack(fp=FOLDER_PAIRS_LIST[target], cns=None, grid=[3, 1], plot_data_range="all", scaling=[1])
